...
Function Cleanup
{
    ...
    {
	    Write-Host "Unable to remove the 'MailApp-openidconnect-v2' . Try deleting manually." -ForegroundColor White -BackgroundColor Red
    }
    ...
    try
    {
        Get-AzureADServicePrincipal -filter "DisplayName eq 'MailApp-openidconnect-v2'" | ForEach-Object {Remove-AzureADServicePrincipal -ObjectId $_.Id -Confirm:$false}
    }
    catch
    {
	    Write-Host "Unable to remove ServicePrincipal 'MailApp-openidconnect-v2' . Try deleting manually from Enterprise applications." -ForegroundColor White -BackgroundColor Red
    }
}
